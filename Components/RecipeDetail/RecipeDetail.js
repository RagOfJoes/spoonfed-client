...
export default memo(({ onLike, onError }) => {
	...
	if (!loading && data?.getRecipeDetail) return <RecipeDetailContent {...data?.getRecipeDetail} onLike={onLike} />;

	if (!loading && !error && router?.query?.recipeSlug && !data?.getRecipeDetail) return <RecipeDetailError />;
