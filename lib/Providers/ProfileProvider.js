...
import { GetUserRecipesQuery, GetUserProfileQuery, GetUserCreationsQuery } from 'graphql/Queries';
...
const ProfileProvider = ({ children }) => {
	...
	const profile = useQuery(GetUserProfileQuery, {
		...
		onError: async (e) => await handleAuthError(e, null, enqueueSnackbar, { message: 'User not found!' }),
	});
	const recipes = useQuery(GetUserRecipesQuery, {
		variables: defaultRecipesVariable,
		skip: router.route !== '/u/[username]',
		onCompleted: () => toggleFetching(false),
		onError: async (e) => await handleAuthError(e, null, enqueueSnackbar, { message: 'User not found!' }),
	});
	const creations = useQuery(GetUserCreationsQuery, {
		...
		onCompleted: () => toggleFetching(false),
		skip: router.route !== '/u/[username]/creations',
		onError: async (e) => await handleAuthError(e, null, enqueueSnackbar, { message: 'User not found!' }),
	});
