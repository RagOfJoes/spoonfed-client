...
export const UserSearchQuery = gql`
  query UserSearchQuery($username: String!, $limit: Int!, $cursor: ID) {
    userSearch(username: $username, limit: $limit, cursor: $cursor)
      @connection(key: "UserSearch", filter: ["username"]) {
      edges {
        sub
        avatar
        username
...
export const GetRecipesForCreationsQuery = gql`
  query GetRecipesForCreationsQuery(
    ...
    getAllRecipes(
      ...
      filters: $filters
      sort: $sort
    ) @connection(key: "RecipesForCreations") {
      pageInfo {
        cursor
        hasNextPage
      }
...
export const GetRecipeDetail = gql`
  query GetRecipeDetail($slug: String!) {
    getRecipeDetail(slug: $slug)
      @connection(key: "RecipeDetail", filter: ["slug"]) {
      ...RecipeDetail
    }
  }
  ${RecipeDetailFragment}
...
export const GetCreationDetailQuery = gql`
  query GetCreationDetailQuery($slug: String!) {
    getCreationDetail(slug: $slug)
      @connection(key: "CreationDetail", filter: ["slug"]) {
      ...CreationDetail
    }
  }
  ${CreationDetailFragment}
